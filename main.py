...
def generate_invoice():
    ...
    doc_name = "new_invoice" + name + datetime.datetime.now().strftime("%d-%m-%Y-%H%M%S") + ".docx"
    doc.save(doc_name)
...
price_spinbox = tkinter.Spinbox(frame, from_=0.0, to=5000, increment=10.0)
...
save_invoice_button = tkinter.Button(
    frame, text="Generate Invoice", command=generate_invoice
)
